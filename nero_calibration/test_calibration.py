...
import numpy as np
import cv2
from core import pose_matrix, matrix_pose, inverse, matrix, tcp_transform, PALM, HAND_BASE, solve
...
class CalibrationTests(unittest.TestCase):
    ...
    def test_stale_feedback_is_rejected(self):
        ...
        with patch('sensors.time.monotonic', side_effect=[0., .1, .6]), patch('sensors.time.sleep'):
    ...
    def test_capture_brackets_pose_and_rejects_motion(self):
        ...
        with patch('calibrate.time.sleep'):
    ...
    def test_collect_existing_directory_requires_explicit_resume(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch('sensors.RealSenseCamera') as camera:
                with self.assertRaises(FileExistsError):
    ...
    def test_collect_resume_appends_next_index_and_filters_old_pose(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch('sensors.RealSenseCamera', return_value=cam), \
                 ...
                 patch('calibrate.capture_sample', side_effect=[(duplicate, image, image),
                                                               (fresh, image, image)]), \
    ...
    def test_resume_rejects_camera_mismatch_before_arm(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            cam.info = {**camera_info, 'camera_matrix': [[601., 0., 320.],
                                                         [0., 600., 240.], [0., 0., 1.]]}
            with patch('sensors.RealSenseCamera', return_value=cam), \
                 patch('sensors.NeroFeedback') as arm:
