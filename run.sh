...
cd "$DICE_ROOT"
if [[ ${1:-} == '--help' || $# == 0 ]]; then
  echo 'Usage: ./run.sh {step|auto|fast|control} [--show] [--execute] [--until place]'
...
mode=$1; shift
case "$mode" in step|auto|fast|control) ;; *) echo 'Mode must be step, auto, fast or control' >&2; exit 2;; esac
