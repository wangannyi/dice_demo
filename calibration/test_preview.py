...
class PreviewTests(unittest.TestCase):
    def test_full_board_window_status_uses_outer_corners(self):
        ui = CollectionPreview.__new__(CollectionPreview)
        ui.board_window = [0, 0, 640, 480]
        camera = Mock()
        camera.K = np.array([[1000., 0., 320.], [0., 1000., 240.], [0., 0., 1.]])
        camera.D = np.zeros(5)
        detector = Mock()
        detector.cfg = {'squares_x': 4, 'squares_y': 5, 'square_length_m': .02}
        pose = pose_matrix([0, 0, .5, 0, 0, 0])
        inside, points = ui.board_window_status(pose, camera, detector)
        self.assertTrue(inside)
        self.assertEqual(points.shape, (4, 2))
        ui.board_window = [0, 0, 470, 480]
        self.assertFalse(ui.board_window_status(pose, camera, detector)[0])
