...
query, and its target is the existing rig candidate, not a calibrated grasp.
"""
import argparse
...
OPEN_TARGET = dict(zip(FINGERS, [18, 3, 0, 0, 0, 0]))


def send_open(hand, *, monotonic=time.monotonic):
    start = monotonic()
...
def run(bus_class, factory, *, execute=False, evidence_provider=host_control_evidence,
        monotonic=time.monotonic, wallclock=time.time, sleep=time.sleep):
    report = {'kind': 'isolated_pregrasp_open', 'execute_selected': execute,
              'target_0_100': OPEN_TARGET, 'target_is_unverified_rig_candidate': True,
              'duration_s': 1., 'arm_target_sent': False, 'finger_close_sent': False,
    ...
    try:
        ...
        for row in baseline:
            report['blockers'].extend(ready_blockers(row))
        ...
        if not execute:
            ...
            report['position_time_interval_s'] = send_open(hand, monotonic=monotonic)
...
def main():
    ...
    with control_lock('/tmp/nero_can0_control.lock'), redirect_stdout(sys.stderr):
        ...
        report = run(bus, factory, execute=args.execute)
