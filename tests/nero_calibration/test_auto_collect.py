...
from nero_calibration.auto_collect import (board_corners, check_path, inside_window, interpolate,
...
class AutoCollectionTests(unittest.TestCase):
    ...
    def test_plan_then_mock_run_writes_all_samples_without_hardware(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('nero_calibration.auto_collect.load_model', return_value=FakeModel()):
            ...
            with patch('nero_calibration.auto_collect.RealSenseCamera', return_value=camera), \
                 patch('nero_calibration.auto_collect.NeroFeedback', return_value=arm), \
                 patch('nero_calibration.auto_collect.CharucoDetector', return_value=detector), \
                 patch('nero_calibration.auto_collect.load_model', return_value=FakeModel()), \
                 patch('nero_calibration.auto_collect.require_arm_ready'), \
                 patch('nero_calibration.auto_collect.live_board'), \
                 patch('nero_calibration.auto_collect.move_and_watch') as move, \
                 patch('nero_calibration.auto_collect.capture_sample', return_value=(sample, frame, frame)):
    ...
    def test_replay_fps_override_keeps_teaching_geometry(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('nero_calibration.auto_collect.RealSenseCamera', return_value=camera) as open_camera, \
                 patch('nero_calibration.auto_collect.NeroFeedback', side_effect=RuntimeError('mock stop')):
