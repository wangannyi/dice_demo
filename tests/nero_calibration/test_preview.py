...
from nero_calibration.calibrate import main, write_new
...
class PreviewTests(unittest.TestCase):
    def test_manual_preview_records_joints_for_each_displayed_frame(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('nero_calibration.sensors.RealSenseCamera', return_value=camera), \
                 patch('nero_calibration.sensors.NeroFeedback', return_value=arm), \
                 patch('nero_calibration.preview.CollectionPreview', return_value=ui), \
    ...
    def test_preview_preserves_rejection_resume_and_saved_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('nero_calibration.sensors.RealSenseCamera', return_value=cam), \
                 ...
                 patch('nero_calibration.calibrate.capture_sample', side_effect=[(sample, image, image),
                                                               (fresh, image, image)]), \
    ...
    def test_missing_x11_fails_before_camera_and_arm_open(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(os.environ, {}, clear=True), \
             patch('nero_calibration.sensors.RealSenseCamera') as camera, patch('nero_calibration.sensors.NeroFeedback') as arm:
    ...
    def test_ui_failure_still_closes_arm_and_camera(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('nero_calibration.sensors.RealSenseCamera', return_value=camera), \
                 patch('nero_calibration.sensors.NeroFeedback', return_value=arm), \
                 patch('nero_calibration.preview.CollectionPreview', return_value=ui), patch('builtins.print'):
