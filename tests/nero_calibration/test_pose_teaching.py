...
from nero_calibration.sensors import NeroFeedback
...
class TeachingTest(unittest.TestCase):
    ...
    def test_requires_new_joint_feedback(self):
        ...
        with patch('nero_calibration.sensors.time.monotonic',side_effect=[0,.1,2]),patch('nero_calibration.sensors.time.sleep'):
...
class ResumeRoiTest(unittest.TestCase):
    def test_only_detection_roi_may_change(self):
        from nero_calibration.calibrate import same_board_for_resume
