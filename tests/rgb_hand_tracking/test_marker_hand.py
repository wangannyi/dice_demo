...
import cv2
import numpy as np
from rgb_hand_tracking.marker_hand import candidate_rects, acceptable, normalized_palm
...
class Tests(unittest.TestCase):
    ...
    def synthetic_run(self, root, manifest, *, shift_after=None, fail_selected_at=None, **options):
        from rgb_hand_tracking.marker_hand import run
        ...
        class FakeBridge:
            ...
            def close(self):
                pass
        with patch('rgb_hand_tracking.marker_hand.Bridge', FakeBridge), patch(
                'rgb_hand_tracking.marker_hand.cv2.aruco.drawDetectedMarkers', wraps=cv2.aruco.drawDetectedMarkers) as draw:
    ...
    def test_invalid_options_fail_before_starting_bridge_or_creating_output(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            with patch('rgb_hand_tracking.marker_hand.Bridge') as bridge:
    ...
    def test_outlier_does_not_redefine_palm(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch('rgb_hand_tracking.marker_hand.Bridge',FakeBridge), patch('rgb_hand_tracking.marker_hand.MarkerTracker.update',return_value=marker):
