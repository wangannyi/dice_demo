...
from cup_grasp_demo.flow.tcp_overlay import draw_tcp, project
...
class TcpOverlayTest(unittest.TestCase):
    ...
    def test_separate_marker_colors_and_closed_hand_reference(self):
        fk = np.eye(4)
        fk[2, 3] = 1
        with patch('cup_grasp_demo.flow.tcp_overlay.load_model') as model:
    ...
    def test_tcp_view_works_at_non_home_and_never_sends_run(self):
        ...
        cfg = ROOT / 'cup_grasp_demo/flow/index_joint_center/config.json'
