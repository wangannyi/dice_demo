...
from cup_grasp_demo.flow.core import ROOT, Screen, cached_screen_geometry, read_json
from cup_grasp_demo.flow.parameters import shake_options
from cup_grasp_demo.flow.shake_tracking import FeedbackMonitor, check_delivery
...
class DurationTest(unittest.TestCase):
    ...
    def test_transient_warning_recovers_but_sustained_error_stops(self):
        ...
        with patch('cup_grasp_demo.flow.shake_tracking.check_feedback', return_value=geometric), \
             patch('cup_grasp_demo.flow.shake_tracking.interpolate_displacement', return_value=0):
...
class TableBatchTest(unittest.TestCase):
    ...
    def test_geometry_cache_is_scoped_to_one_run(self):
        ...
        with patch.object(Screen, 'add'), patch('cup_grasp_demo.flow.core.load_model') as load:
...
class ExecutorDurationTest(unittest.TestCase):
    def simulate(self, fail_after=None):
        ...
        from cup_grasp_demo.flow import shake_execution as sdk
