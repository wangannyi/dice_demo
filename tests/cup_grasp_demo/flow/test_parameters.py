...
from cup_grasp_demo.flow.core import ROOT, configured_tcp, load_config, read_json
from cup_grasp_demo.flow.grasp import make_grasp_plan, observed_scene, options
from cup_grasp_demo.flow.grasp_execution import send_closure, validate_sequence
...
class ParametersTest(unittest.TestCase):
    def setUp(self):
        self.path = ROOT / 'cup_grasp_demo/flow/index_joint_center/config.json'
    ...
    def test_custom_targets_reach_sdk_arguments(self):
        ...
        def sleep(delta):
            clock[0] += delta
        with patch('cup_grasp_demo.flow.grasp_execution.fresh_current', return_value={}):
            for stage in stages:
