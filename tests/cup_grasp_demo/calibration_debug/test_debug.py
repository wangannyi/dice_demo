...
from cup_grasp_demo.calibration_debug.debug import validate_plan
from cup_grasp_demo.calibration_debug.hardware import validate_start
from nero_calibration.core import pose_matrix
from nero_revo2_control.kinematics import load_model

HERE = Path(__file__).resolve().parents[3] / 'cup_grasp_demo/calibration_debug'
...
class DebugTest(unittest.TestCase):
    ...
    def test_j7_rotation_is_in_fk_without_mutating_local_tcp(self):
        arm = load_model()
        q = np.array(json.loads((HERE / 'home_reference.json').read_text())['joints_rad'])
    ...
    def test_preview_move_has_no_hardware_calls(self):
        from cup_grasp_demo.calibration_debug.debug import execute_plan
        ...
        with patch('cup_grasp_demo.calibration_debug.debug.read_json', return_value=plan), \
             patch('cup_grasp_demo.calibration_debug.debug.verify_session', return_value=({}, {})), \
             patch('cup_grasp_demo.calibration_debug.debug.validate_plan'), \
             patch('cup_grasp_demo.calibration_debug.debug.make_plan', return_value=copy.deepcopy(plan)), \
             patch('cup_grasp_demo.calibration_debug.debug.bridge') as hardware, \
             patch('cup_grasp_demo.calibration_debug.debug.capture_rgbd') as camera:
