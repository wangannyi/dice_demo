...
sys.path[:0] = [str(ROOT)]
from cup_grasp_demo.flow import shake_execution as shared
from cup_grasp_demo.flow.joint_delivery import take_js_control, fresh_js_hold
from cup_grasp_demo.flow.joint_profile import (
    KIND,
    options,
    make_plan,
    command_values,
...
def validate_request(request, now):
    ...
    if request.get('load_context') == 'green_cup_held':
        from cup_grasp_demo.flow.green_shake_validation import validate_held_request
