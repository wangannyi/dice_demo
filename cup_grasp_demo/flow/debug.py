...
from cup_grasp_demo.flow.cup_recheck import verify_cup  # noqa: E402
from cup_grasp_demo.flow.cup_selection import select_cup  # noqa: E402
from cup_grasp_demo.flow.tcp_overlay import draw_tcp  # noqa: E402
...
def capture(args, *, prepared=None, snapshot=None):
    ...
    if prepared is not None:
        from cup_grasp_demo.flow.grasp import scene_from_points
...
def config_check(args):
    ...
    from cup_grasp_demo.flow.parameters import approach_options, closure_targets
    cfg = load_config(args.config)
...
def main(argv=None):
    ...
    if args.command == 'pipeline':
        ...
        from cup_grasp_demo.flow.pipeline_runner import run
        return run(args) if args.status else dispatch(args, run)
    if args.command == 'shake':
        from cup_grasp_demo.flow.shake_cli import execute
        return dispatch(args, execute)
    if args.command == 'shake-plan':
        from cup_grasp_demo.flow.shake_cli import plan as shake_plan
