...
from cup_grasp_demo.calibration_debug.core import ROOT, digest, load_config, read_json
from cup_grasp_demo.calibration_debug.cup_selection import choose_candidate, select_cup, selection_options
from cup_grasp_demo.calibration_debug.grasp import observed_scene
...
class CupSelectionTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.config_path = ROOT / 'cup_grasp_demo/calibration_debug/index_joint_center/config.json'
    ...
    def test_no_candidate_or_plane_failure_saves_diagnostics(self):
        for effect in ('empty', 'bad_plane'):
            with tempfile.TemporaryDirectory() as temp, patch(
                    'cup_grasp_demo.calibration_debug.cup_selection.depth_candidates') as detect:
                if effect == 'empty':
                    detect.return_value = ([], [])
                else:
